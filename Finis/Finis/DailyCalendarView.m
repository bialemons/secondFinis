...
-(UILabel *)dateLabel
{
    if(!_dateLabel) {
        ...
        [_dateLabel setFont:[UIFont fontWithName:@"Helvetica" size:DATE_LABEL_FONT_SIZE]];
    }
...
-(void)markSelected:(BOOL)blnSelected
{
    ...
    if([self.date isDateToday]){
        ...
        //marca cor da célula do dia
        self.dateLabelContainer.backgroundColor = (blnSelected)?[UIColor whiteColor]: [UIColor colorWithHex:0x4ECDC4];
        
        //cor do dia(numero)
        self.dateLabel.textColor = (blnSelected)?[UIColor colorWithHex:0x7BD1FF]:[UIColor whiteColor];
    } else
{
        self.dateLabelContainer.backgroundColor = (blnSelected)?[UIColor whiteColor]: [UIColor clearColor];
        
        self.dateLabel.textColor = (blnSelected)?[UIColor colorWithRed:78.0/255.0 green:205.0/255.0 blue:196.0/255.0 alpha:1.0]:[self colorByDate];
    }
}

// aqui é a cor dos dias passados
-(UIColor *)colorByDate
{
    return [self.date isPastDate]?[UIColor colorWithHex:0x7BD1FF]:[UIColor whiteColor];
}

// func que faz reconhecer o tap na célula
